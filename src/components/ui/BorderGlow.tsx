...
function parseHSL(hslStr: string) {
  ...
  if (!match) {
    const matchAlt = hslStr.match(/([\d.]+)\s*([\d.]+)%?\s*([\d.]+)%?/);
    if (!matchAlt) return { h: 270, s: 50, l: 60 };
    return { h: parseFloat(matchAlt[1]), s: parseFloat(matchAlt[2]), l: parseFloat(matchAlt[3]) };
  }
...
function buildBoxShadow(glowColor: string, intensity: number) {
  ...
  return layers.map(([x, y, blur, spread, alpha, inset]) => {
    const a = Math.min(alpha * (intensity as number), 100);
